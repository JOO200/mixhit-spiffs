...
// Diese Funktion dient als Struckt zum Speichern eines Cocktails
function Cocktail(pCocktail, pVolume) { 
...
var CocktailSpeicher = (function () {
    ...
    return {        // im return können alle öffentlichen Funktionen und variabeln angelegt werden
        searchID : function (id) {  // returnt den Cocktail an der Stelle "id"
        ...
        addCocktail : function (pCocktail) {    // fügt einen Cocktail der Liste hinzu und gibt die Position des neuen Cocktails zurück
            Speicher.push(pCocktail); 
            return Speicher.length - 1;          
        },

        deleteAll : function () {   // entfernt alle Elemente aus dem Array
            Speicher = new Array();    
        }
    }
})();
...
function updateInformation(Speicher) {  // Diese Funktion verarbeitet die Informationen, die über die ajax Anfrage eingetroffen sind
    ...
    ListAlkoholisch.removeChild(ListAlkoholisch.childNodes[0]); // entfert die darin enthaltene Liste mit Cocktails
    ...
    Speicher.AlkoholischeCocktails.forEach(element => {         // for each Schleife für alkoholische Cocktails
        ...
        '</p></div><button class="btn col-2 m2-col-3" onclick="small_button_click(' 
        + CocktailSpeicher.addCocktail(new Cocktail(element, 120)) +    // neuer Cocktail mit 120 ml wird im Speicher angelegt und die Nummer in den Aufruf des Buttons eingetragen
        ')">120 ml</button><button class="btn col-2 m2-col-3" onclick="big_button_click('
        + CocktailSpeicher.addCocktail(new Cocktail(element, 250)) +    // neuer Cocktail mit 250 ml wird im Speicher angelegt und die Nummer in den Aufruf des Buttons eingetragen
        ')">250 ml</button></div>';
    ...
    ListNichtAlkoholisch.removeChild(ListNichtAlkoholisch.childNodes[0]);   // entfert die darin enthaltene Liste mit Cocktails
...
function small_button_click(pID) {  // Diese Funktion wird aus dem HTML Dokument aufgerufen, wenn auf den entsprechenden Button geklickt wird
    ...
    window.alert("Vielen Dank für deine Bestellung!");          // erzeugt Benachrichtigungsfenster im Browser, damit der Benutzer informiert wird
}

function big_button_click(pID) {    // Diese Funktion wird aus dem HTML Dokument aufgerufen, wenn auf den entsprechenden Button geklickt wird
    ajax.open("POST","cocktail",true);                          // erstellt neue ajax Anfage
    ajax.setRequestHeader("Content-type", "application/json");  // legt Parameter im Header der Anfage fest
    ajax.send(JSON.stringify(CocktailSpeicher.searchID(pID)));  // sendet Anfage mit Cocktail in JSON Format als Inhalt 
    window.alert("Vielen Dank für deine Bestellung!");          // erzeugt Benachrichtigungsfenster im Browser, damit der Benutzer informiert wird
